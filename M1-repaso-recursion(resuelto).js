...
function hayRuta(rutas, salida, destino) {
  // Tu código aquí:
  console.log(rutas[salida]);
  if(rutas[salida]) {
    for (const ciudad of rutas[salida]) {
      console.log(ciudad);
      if(ciudad === destino) return true;
      if(hayRuta(rutas, ciudad, destino)) return true;
    }
    //if(rutas[salida].includes(destino)) return true;
  }
  return false;
}
